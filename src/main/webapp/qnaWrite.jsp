...
<br/><br/><br/>
   ...
   <script type="text/javascript">
      ...
      function chkEmpty() {
		  var comSubmit = new ComSubmit();
        if ($('input[name=QNA_NAME]').val() === undefined || $('input[name=QNA_NAME]').val() === "" || $('input[name=QNA_NAME]').val() === NULL) {
            alert("이름을 입력해주세요!");
			comSubmit.setUrl("<c:url value='/qna/openQnaWrite.do' />");
			comSubmit.submit();
        } else if ($('input[name=QNA_TITLE]').val() === undefined || $('input[name=QNA_TITLE]').val(C) === "" || $('input[name=QNA_TITLE]').val() === NULL) {
            alert("제목을 입력해주세요!");
			comSubmit.setUrl("<c:url value='/qna/openQnaWrite.do' />");
			comSubmit.submit();
        } else if ($('textarea[name=QNA_CONTENT]').val() === undefined || $('textarea[name=QNA_CONTENT]').val() === "" || $('textarea[name=QNA_CONTENT]').val() === NULL) {
            alert("내용을 입력해주세요!");
			comSubmit.setUrl("<c:url value='/qna/openQnaWrite.do' />");
			comSubmit.submit();
        } else if ($('input[name=MEMBER_EMAIL]').val() === undefined || $('input[name=MEMBER_EMAIL]').val() === "" || $('input[name=MEMBER_EMAIL]').val() === NULL) {
            alert("이메일을 입력해주세요!");
			comSubmit.setUrl("<c:url value='/qna/openQnaWrite.do' />");
			comSubmit.submit();
        }  else if ($('input[name=QNA_SECRET]').val() === undefined || $('input[name=QNA_SECRET]').val() === "" || $('input[name=QNA_SECRET]').val() === NULL ) {
            alert("비밀번호를 입력해주세요!");
			comSubmit.setUrl("<c:url value='/qna/openQnaWrite.do' />");
			comSubmit.submit();
        }
