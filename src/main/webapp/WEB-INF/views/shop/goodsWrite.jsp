...
function fn_insertBoard() { // 유효성체크
    ...
    if(CKEDITOR.instances.GOODS_CONTENT.getData() =='' 
 	...
 	// 상품원가 
    if(!$("#GOODS_ORIGIN_PRICE").val() || isNaN($("#GOODS_ORIGIN_PRICE").val())){
        alert("상품원가를 입력해주세요.");
        $("#GOODS_ORIGIN_PRICE").focus();
        return false;
    }
 	// 할인가
    if(!$("#GOODS_SALE_PRICE").val() || isNaN($("#GOODS_SALE_PRICE").val())){
        alert("상품할인가를 입력해주세요.");
        $("#GOODS_SALE_PRICE").focus();
        return false;
    }
 	// 판매가
    if(!$("#GOODS_SELL_PRICE").val() || isNaN($("#GOODS_SELL_PRICE").val())){
        alert("판매가를 입력해주세요.");
    ...
    if(!$("#GOODS_ATT_COLOR").val()){
	 ...
	 // 상품 수량
    if(!$("#GOODS_ATT_AMOUNT").val() || isNaN($("#GOODS_ATT_AMOUNT").val())){
        alert("상품수량을 입력해주세요.");
