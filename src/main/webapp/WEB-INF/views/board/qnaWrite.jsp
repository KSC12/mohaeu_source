...
<br/><br/><br/>
   ...
   <input type="hidden" id="member_no" name="MEMBER_NO" value="${SESSION_NO}">
      ...
      <div id="" class="">
      <p>
         <label><input type="hidden" name="QNA_SECRET" value="1" style="margin-right:5px;" id="qna_secret" checked=""><span></span>비밀글(관리자만 볼수 있습니다.)</label>
      </p>
      </div>
      <br>
      <p>
   ...
   <script type="text/javascript">
      ...
      function fn_insertQna(){     
		if ($('input[name=QNA_NAME]').val() === undefined || $('input[name=QNA_NAME]').val() === "") {
            alert("이름을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('input[name=QNA_TITLE]').val() === undefined || $('input[name=QNA_TITLE]').val() === "") {
            alert("제목을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('textarea[name=QNA_CONTENT]').val() === undefined || $('textarea[name=QNA_CONTENT]').val() === "") {
            alert("내용을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('input[name=MEMBER_EMAIL]').val() === undefined || $('input[name=MEMBER_EMAIL]').val() === "") {
            alert("이메일을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('input[name=QNA_PASSWD]').val() === undefined || $('input[name=QNA_PASSWD]').val() === "") {
            alert("비밀번호를 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else {
      ...
      /*function chkEmpty(){  
        if ($('input[name=QNA_NAME]').val() === undefined || $('input[name=QNA_NAME]').val() === "") {
            ...
            alert("이메일을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('input[name=QNA_CATEGORY]').val() === undefined || $('input[name=QNA_CATEGORY]').val() === "") {
            alert("문의유형을 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        } else if ($('input[name=QNA_SECRET]').val() === undefined || $('input[name=QNA_SECRET]').val() === "") {
            alert("비밀번호를 입력해주세요!");
            location.reload();  // 페이지 새로 고침
        }
	}*/
