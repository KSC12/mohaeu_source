...
public class AdminMainController {
	...
	//회원 목록
		@RequestMapping(value="/order_admin.do")
		public ModelAndView order_admin(CommandMap commandMap) throws Exception {
			
			ModelAndView mv = new ModelAndView("admin/order_admin");
			
			List<Map<String, Object>> order_admin_list = adminMainService.selectOrderList(commandMap.getMap());
			
			mv.addObject("order_admin_list", order_admin_list);
			
			System.out.println("멤버리스트"+order_admin_list);
	...
	//회원 목록 페이징
		@RequestMapping(value="/order_admin_list.do")
		public ModelAndView order_admin_list(CommandMap commandMap) throws Exception {
			...
			if(order_admin_list.size()>0) {
				mv.addObject("TOTAL", order_admin_list.get(0).get("TOTAL_COUNT"));
			}else {
				mv.addObject("TOTAL", 0);
			}
